...
#include "interfaces/srv/servo.hpp" //Set Motor servo on/off
...
public:
	TETRA() : Node("tetra")
	{
		...
		servo_srv = create_service<interfaces::srv::Servo>(
        	"servo_cmd", 
		std::bind(&TETRA::Servo_Command, this, std::placeholders::_1, std::placeholders::_2));
...
private:
	...
	rclcpp::Service<interfaces::srv::Servo>::SharedPtr servo_srv;
	...
	bool Servo_Command(
		const std::shared_ptr<interfaces::srv::Servo::Request> request, 
		const std::shared_ptr<interfaces::srv::Servo::Response> response)
	{
		bool bResult = false;

		int m_idata = request->data;
		if(m_idata == 1)
		{
			bPosition_mode_flag = false;
			//Velocity Mode
