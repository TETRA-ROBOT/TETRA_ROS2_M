...
typedef struct ROBOT {
  ...
  // move result
  int m_iMovebase_Result = 0; // Success: 3 | Aborted: 4 | Cancel: 1 | Unknown:
                              // 0
...
public:
  TETRA_SERVICE() : Node("tetra_service") {
    ...
    cmd_vel_publisher =
        this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel_nav", 10);
    pose_reset_publisher =
        this->create_publisher<std_msgs::msg::Int32>("pose_reset", 10);
    initial_pose_publisher =
        this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
            "initialpose", 10);
    movebase_publisher =
        this->create_publisher<std_msgs::msg::Int32>("movebase", 10);
    footprint_publisher =
        this->create_publisher<geometry_msgs::msg::Pose>("footprint", 10);
    // bumper_pointcloud_publisher =
    // this->create_publisher<sensor_msgs::msg::PointCloud2>("bumper_pointcloud",
    // 10);
    ...
    apriltag_subscriber =
        this->create_subscription<apriltag_msgs::msg::AprilTagDetectionArray>(
            "detections", 10,
            std::bind(&TETRA_SERVICE::ApriltagCallback, this, _1));

    sick_scan_subscriber =
        this->create_subscription<sensor_msgs::msg::LaserScan>(
            "scan", rclcpp::SensorDataQoS(),
            std::bind(&TETRA_SERVICE::SickCallback, this, _1));

    cygbot_scan_subscriber =
        this->create_subscription<sensor_msgs::msg::LaserScan>(
            "scan2", rclcpp::SensorDataQoS(),
            std::bind(&TETRA_SERVICE::CygbotCallback, this, _1));
    ...
    tetra_battery_subscriber = this->create_subscription<std_msgs::msg::Int32>(
        "tetra_battery", 1,
        std::bind(&TETRA_SERVICE::BatteryCallback, this, _1));

    docking_status_subscriber = this->create_subscription<std_msgs::msg::Int32>(
        "docking_status", 1,
        std::bind(&TETRA_SERVICE::DockingStatusCallback, this, _1));

    conveyor_sensor_subscriber =
        this->create_subscription<std_msgs::msg::Int32>(
            "conveyor_sensor", 1,
            std::bind(&TETRA_SERVICE::ConveyorSensorCallback, this, _1));

    conveyor_movement_subscriber =
        this->create_subscription<std_msgs::msg::Int32>(
            "conveyor_movement", 1,
            std::bind(&TETRA_SERVICE::ConveyorMovementCallback, this, _1));

    joy_subscriber = this->create_subscription<sensor_msgs::msg::Joy>(
        "joy", rclcpp::SensorDataQoS(),
        std::bind(&TETRA_SERVICE::joyCallback, this, _1));

    particlecloud_subscriber =
        this->create_subscription<nav2_msgs::msg::ParticleCloud>(
            "particle_cloud", rclcpp::SensorDataQoS(),
            std::bind(&TETRA_SERVICE::AMCL_PaticleCloud_Callback, this,
                      _1)); // particle_cloud

    cmd_vel_subscriber = this->create_subscription<geometry_msgs::msg::Twist>(
        "cmd_vel", rclcpp::SensorDataQoS(),
        std::bind(&TETRA_SERVICE::cmd_vel_Callback, this, _1));

    initialpose_subscriber = this->create_subscription<
        geometry_msgs::msg::PoseWithCovarianceStamped>(
        "initialpose", rclcpp::SensorDataQoS(),
        std::bind(&TETRA_SERVICE::Initialpose_Callback, this, _1));

    map_subscriber = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        "map", rclcpp::SensorDataQoS(),
        std::bind(&TETRA_SERVICE::map_Callback, this, _1));

    // Service
    // list///////////////////////////////////////////////////////////////////////////////////////
    docking_cmd_srv = create_service<interfaces::srv::DockingControl>(
        "docking_cmd", std::bind(&TETRA_SERVICE::Docking_Command, this,
                                 std::placeholders::_1, std::placeholders::_2));

    docking_stop_cmd_srv = create_service<interfaces::srv::DockingStop>(
        "docking_stop_cmd",
        std::bind(&TETRA_SERVICE::Docking_Stop_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    save_map_cmd_srv = create_service<interfaces::srv::SaveMap>(
        "save_map_cmd",
        std::bind(&TETRA_SERVICE::Save_Map_Command, this, std::placeholders::_1,
                  std::placeholders::_2));

    delete_map_cmd_srv = create_service<interfaces::srv::DeleteMap>(
        "delete_map_cmd",
        std::bind(&TETRA_SERVICE::Delete_Map_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    run_mapping_cmd_srv = create_service<interfaces::srv::RunMapping>(
        "run_mapping_cmd",
        std::bind(&TETRA_SERVICE::Mapping_Mode_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    run_navigation_cmd_srv = create_service<interfaces::srv::RunNavigation>(
        "run_navigation_cmd",
        std::bind(&TETRA_SERVICE::Navigation_Mode_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    get_information_cmd_srv = create_service<interfaces::srv::GetInformation>(
        "get_information_cmd",
        std::bind(&TETRA_SERVICE::Get_Information_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    set_location_cmd_srv = create_service<interfaces::srv::SetLocation>(
        "set_location_cmd",
        std::bind(&TETRA_SERVICE::Set_Location_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    goto_location_cmd_srv = create_service<interfaces::srv::GotoLocation>(
        "goto_location_cmd",
        std::bind(&TETRA_SERVICE::Goto_Location_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    goto_location2_cmd_srv = create_service<interfaces::srv::GotoLocation2>(
        "goto_location2_cmd",
        std::bind(&TETRA_SERVICE::Goto_Location2_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    goto_cancel_cmd_srv = create_service<interfaces::srv::GotoCancel>(
        "goto_cancel_cmd",
        std::bind(&TETRA_SERVICE::Goto_Cancel_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    set_maxspeed_cmd_srv = create_service<interfaces::srv::SetMaxspeed>(
        "set_maxspeed_cmd",
        std::bind(&TETRA_SERVICE::Set_Maxspeed_Command, this,
                  std::placeholders::_1, std::placeholders::_2));

    // Client
    // list///////////////////////////////////////////////////////////////////////////////////////
    led_control_client =
        this->create_client<interfaces::srv::LedControl>("led_cmd");
    led_toggle_control_client =
        this->create_client<interfaces::srv::LedToggleControl>(
            "led_toggle_cmd");
    toggle_on_client =
        this->create_client<interfaces::srv::ToggleOn>("turn_on_cmd");
    imu_reset_client =
        this->create_client<interfaces::srv::ImuReset>("all_data_reset");
    // request_nomotion_update Service Client
    request_nomotion_update_client =
        this->create_client<std_srvs::srv::Empty>("request_nomotion_update");
    // Clear Costmap Service Client
    clear_entire_global_costmap_client =
        this->create_client<nav2_msgs::srv::ClearEntireCostmap>(
            "global_costmap/clear_entirely_global_costmap");
    clear_entire_local_costmap_client =
        this->create_client<nav2_msgs::srv::ClearEntireCostmap>(
            "local_costmap/clear_entirely_local_costmap");
    // robot_localization Service Client
    set_pose_client_ =
        this->create_client<robot_localization::srv::SetPose>("set_pose");

    // Set Max Speed parameter client
    set_speed_parameter_client_ =
    ...
    timer_ = this->create_wall_timer(
        50ms, std::bind(&TETRA_SERVICE::DockingThread_function, this));
    // TF Calc Timer
    TF_timer_ = this->create_wall_timer(
        20ms, std::bind(&TETRA_SERVICE::TF_CALC_Timer, this));
    Footprint_timer_ = this->create_wall_timer(
        20ms, std::bind(&TETRA_SERVICE::Footprint_CALC_Timer, this));
  }
  ...
  rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr
      tetra_battery_subscriber;
  rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr
      docking_status_subscriber;
  rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr
      conveyor_sensor_subscriber; // Option
  rclcpp::Subscription<std_msgs::msg::Int32>::SharedPtr
      conveyor_movement_subscriber; // Option
  rclcpp::Subscription<sensor_msgs::msg::Joy>::SharedPtr
      joy_subscriber; // joystick
  rclcpp::Subscription<nav2_msgs::msg::ParticleCloud>::SharedPtr
      particlecloud_subscriber; // AMCL particlecloud Subscribe
  rclcpp::Subscription<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr
      initialpose_subscriber; // 2D Pose Estimate
  ...
  // AMCL_
  void AMCL_PaticleCloud_Callback(const nav2_msgs::msg::ParticleCloud::SharedPtr
                                      msg) // nav2_msgs::msg::ParticleCloud
  {
    ...
    printf("## [2D Pose Estimate] AMCL_PaticleCloud_Callback ## \n");
    if (m_iParticleCloud_size > 501 && _pDynamic_param.m_linear_vel == 0.0 &&
        _pDynamic_param.m_angular_vel == 0.0 &&
      ...
      if (_pFlag_Value.m_bFlag_nomotion) {
        while (!request_nomotion_update_client->wait_for_service(1s)) {
          ...
          RCLCPP_INFO_STREAM(this->get_logger(),
                             "[AMCL_callback]service nomotion update not "
  ...
  void ApriltagCallback(
    ...
    if (size != 0) // detection maker
    {
      ...
      string strTF_frame_name =
          _pAR_tag_pose.m_strFamily + to_string(_pAR_tag_pose.m_iApril_tag_id);
      if (!tf2_buffer_->canTransform("camera", strTF_frame_name,
                                     tf2::TimePointZero)) {
        return;
      }
      // TF
      ...
      transformStamped = tf2_buffer_->lookupTransform(
          "camera", strTF_frame_name, tf2::TimePointZero);

      // position
      _pAR_tag_pose.m_dApril_tag_pose_x =
          transformStamped.transform.translation.x;
      _pAR_tag_pose.m_dApril_tag_pose_y =
          transformStamped.transform.translation.y;
      _pAR_tag_pose.m_dApril_tag_pose_z =
          transformStamped.transform.translation.z;
      // rotation//
      _pAR_tag_pose.m_dApril_tag_orientation_x =
          transformStamped.transform.rotation.x;
      _pAR_tag_pose.m_dApril_tag_orientation_y =
          transformStamped.transform.rotation.y;
      _pAR_tag_pose.m_dApril_tag_orientation_z =
          transformStamped.transform.rotation.z;
      _pAR_tag_pose.m_dApril_tag_orientation_w =
      ...
      m.getRotation(q);
      // rotation Matrix -> rpy
      m.getRPY(_pAR_tag_pose.m_dApril_tag_roll,
               _pAR_tag_pose.m_dApril_tag_pitch,
               _pAR_tag_pose.m_dApril_tag_yaw);
      // Transform Axis
      ...
      if (_pAR_tag_pose.m_dPositioning_Angle > 0.0) {
        ...
        if (_pAR_tag_pose.m_transform_old_pose_y > 0) {
          _pAR_tag_pose.m_transform_pose_y =
              _pAR_tag_pose.m_transform_old_pose_y * -1.0;
        } else {
          _pAR_tag_pose.m_transform_pose_y =
  ...
  void SickCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    for (int i = C_minIndex; i < C_maxIndex; i++) {
      if ((msg->ranges[i] <= C_minVal) && (msg->ranges[i] >= msg->range_min) &&
  ...
  void CygbotCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    // msg->ranges[cygbot_closestIndex]);
  ...
  void BumperCallback(const std_msgs::msg::Int32::SharedPtr msg) {
    ...
    if (_pRobot.m_iCallback_Bumper != 0) {
      ...
      // pointCloudMsg.fields.emplace_back("x", 0,
      // sensor_msgs::msg::PointField::FLOAT32, 1);
      // pointCloudMsg.fields.emplace_back("y", 4,
      // sensor_msgs::msg::PointField::FLOAT32, 1);
      // pointCloudMsg.fields.emplace_back("z", 8,
      // sensor_msgs::msg::PointField::FLOAT32, 1);
      ...
      // pointCloudMsg.fields.emplace_back("x", 0,
      // sensor_msgs::msg::PointField::FLOAT32, 0);
      // pointCloudMsg.fields.emplace_back("y", 4,
      // sensor_msgs::msg::PointField::FLOAT32, 0);
      // pointCloudMsg.fields.emplace_back("z", 8,
      // sensor_msgs::msg::PointField::FLOAT32, 0);
    ...
    // // Resize the data array and fill it with zeros (replace this with your
    // own data) pointCloudMsg.data.resize(pointCloudMsg.row_step *
    // pointCloudMsg.height, 0);
  ...
  // ///////////////////////////////////////////////////////////////
  void LedControl_callback(
      rclcpp::Client<interfaces::srv::LedControl>::SharedFuture future) {
    // RCLCPP_INFO(this->get_logger(), "response LedControl_callback");
  }
  void LedToggleControl_callback(
      rclcpp::Client<interfaces::srv::LedToggleControl>::SharedFuture future) {
    // RCLCPP_INFO(this->get_logger(), "response LedToggleControl_callback");
  }
  void ToggleOn_callback(
      rclcpp::Client<interfaces::srv::ToggleOn>::SharedFuture future) {
    // RCLCPP_INFO(this->get_logger(), "response ToggleOn_callback");
  }

  void ImuReset_callback(
      rclcpp::Client<interfaces::srv::ImuReset>::SharedFuture future) {
  ...
  bool Depart_Station2Move() {
    bool bResult = false;
    if (_pAR_tag_pose.m_transform_pose_x <= 0.65 &&
        _pAR_tag_pose.m_iApril_tag_id != -1) // 650mm depart move
    {
      ...
      // Nav Goal call///////////////////////////////////////////////////////
      Set_goal(_pGoal_pose.goal_positionX, _pGoal_pose.goal_positionY,
               _pGoal_pose.goal_positionZ, _pGoal_pose.goal_quarterX,
               _pGoal_pose.goal_quarterY, _pGoal_pose.goal_quarterZ,
  ...
  void Reset_EKF_SetPose() {
    // Reset robot localization reset
    // call/////////////////////////////////////////////
    auto request =
  ...
  bool Marker_Reset_Robot_Pose() {
    ...
    string landmark_name =
        "marker_" + std::to_string(_pAR_tag_pose.m_iApril_tag_id);
    ...
    if (fp != NULL) // File Open Check
    {
      ...
      initPose.pose.pose.orientation.z =
          -1.0 * _pLandMarkPose.init_orientation_z;
  ...
  void Reset_Robot_Pose() {
    if (_pRobot.HOME_id ==
        _pAR_tag_pose.m_iApril_tag_id) // Same as Home ID... Loop
    {
      _pFlag_Value.m_bFlag_nomotion = false;
      // IMU reset//
      ImuReset_Call();
  ...
  void TF_CALC_Timer() {
    try {
      ...
      geometry_msgs::msg::TransformStamped transformStamped;
      transformStamped = tf2_buffer_->lookupTransform("odom", "base_footprint",
                                                      tf2::TimePointZero);
      ...
      m.getRotation(q);
      // rotation Matrix -> rpy
      m.getRPY(_pTF_pose.pose_euler_roll, _pTF_pose.pose_euler_pitch,
               _pTF_pose.pose_euler_yaw);
      _pTF_pose.theta_deg = _pTF_pose.pose_euler_yaw * (180.0 / M_PI);

    } catch (tf2::TransformException &ex) {
      RCLCPP_WARN(this->get_logger(),
                  "Could not transform odom to footprint: %s", ex.what());
    }
  }
  ...
  void Footprint_CALC_Timer() {
    try {
      rclcpp::Time now = this->get_clock()->now();
      geometry_msgs::msg::TransformStamped transformStamped;
      transformStamped = tf2_buffer_->lookupTransform("map", "base_footprint",
                                                      tf2::TimePointZero);
      ...
      // RCLCPP_WARN(this->get_logger(), "Could not transform map to footprint:
      // %s", ex.what());
    }
  }

  bool SaveLocation(string str_location, double m_dPoseTFx, double m_dPoseTFy,
                    double m_dPoseTFz, double m_dPoseTFqx, double m_dPoseTFqy,
                    double m_dPoseTFqz, double m_dPoseTFqw) {
    bool bResult = false;
    ...
    if (fp == NULL) {
      ...
      fprintf(fp, "0,%lf,%lf,%lf,%lf,%lf,%lf,%lf \n", m_dPoseTFx, m_dPoseTFy,
              m_dPoseTFz, m_dPoseTFqx, m_dPoseTFqy, m_dPoseTFqz, m_dPoseTFqw);
      fclose(fp);
      bResult = true;
    }
  ...
  bool OpenLocationFile(string str_location) {
    ...
    if (str_location == "HOME") {
      ...
      if (fp != NULL) // File Open Check
      {
        ...
        RCLCPP_INFO(this->get_logger(), "File Open Fail Error!: %s",
                    str_location.c_str());
        bResult = false;
      }
    }
  ...
  bool ChargingStation_tracking(bool bOn, int marker_id) {
    ...
    if (bOn) {
      ...
      if (_pAR_tag_pose.m_iApril_tag_id == marker_id) {
        m_fdistance = sqrt(_pAR_tag_pose.m_transform_old_pose_x *
        ...
        if (m_fdistance > 0.65) {
          ...
          cmd.angular.z = -1.0 *
                          atan2(_pAR_tag_pose.m_transform_old_pose_y,
                                _pAR_tag_pose.m_transform_old_pose_x) /
                          1.25;
          ...
          if (_pAR_tag_pose.m_target_yaw <= 0.0174533 &&
              _pAR_tag_pose.m_target_yaw >= -0.0174533) //+- 1.0deg
          {
    ...
    if (_pAR_tag_pose.m_target_yaw > 0) //[++dir]
    {
      if (m_iRotation_cnt <= 50) {
        if (_pFlag_Value.m_bFlag_Obstacle_Center) {
          ...
          cmd.angular.z = 0.0;
          cmd_vel_publisher->publish(cmd);
          printf("[Stop]: Obstacle Detections! (Front: %d ) \n",
                 _pFlag_Value.m_bFlag_Obstacle_Center);
        } else {
          // rotation move
  ...
  bool Docking_Command(
      ...
      const std::shared_ptr<interfaces::srv::DockingControl::Response>
          response) {
    bool bResult = false;
  ...
  bool Delete_Map_Command(
    ...
    m_strMapName_yaml = "/home/tetra/ros2_ws/src/tetra_navigation2/maps/" +
                        request->map_name + ".yaml";
    m_strMapName_pgm = "/home/tetra/ros2_ws/src/tetra_navigation2/maps/" +
                       request->map_name + ".pgm";
  ...
  bool Navigation_Mode_Command(
      ...
      const std::shared_ptr<interfaces::srv::RunNavigation::Response>
          response) {
    bool bResult = false;
  ...
  bool Get_Information_Command(
      ...
      const std::shared_ptr<interfaces::srv::GetInformation::Response>
          response) {
    bool bResult = false;
  ...
  bool Set_Location_Command(
    ...
    if (request->location == "HOME") {
      ...
      response->command_result = SaveLocation(
          request->location, _pTF_pose.poseTFx, _pTF_pose.poseTFy,
          _pTF_pose.poseTFz, _pTF_pose.poseTFqx, _pTF_pose.poseTFqy,
          _pTF_pose.poseTFqz, _pTF_pose.poseTFqw);
  ...
  void Set_goal(double position_x, double position_y, double position_z,
    ...
    send_goal_options.feedback_callback =
        std::bind(&TETRA_SERVICE::feedbackCallback, this, std::placeholders::_1,
                  std::placeholders::_2);
    send_goal_options.result_callback =
        std::bind(&TETRA_SERVICE::resultCallback, this, std::placeholders::_1);

    // Send Goal//
    auto parameters = std::vector<rclcpp::Parameter>();
    // parameters.emplace_back(rclcpp::Parameter("FollowPath.desired_linear_vel",
  ...
  bool Goto_Location_Command(
    ...
    if (_pRobot.m_iCallback_Charging_status <= 1 &&
        (_pAR_tag_pose.m_iApril_tag_id == -1 ||
         _pAR_tag_pose.m_transform_pose_x <= 0.5)) // Nomal
    {
  ...
  bool Goto_Location2_Command(
      ...
      const std::shared_ptr<interfaces::srv::GotoLocation2::Response>
          response) {
    bool bResult = false;
    ...
    if (_pRobot.m_iCallback_Charging_status > 1) {
      ...
      bResult = true;
    } else if ((_pAR_tag_pose.m_iApril_tag_id == -1 ||
                _pAR_tag_pose.m_transform_pose_x >= 0.85)) // Nomal
    {
      RCLCPP_INFO(get_logger(), "Goto Nomal Loop !!");

      // Nav Goal call///////////////////////////////////////////////////////
      Set_goal(request->pose_x, request->pose_y, request->pose_z,
               request->pose_qx, request->pose_qy, request->pose_qz,
               request->pose_qw);
  ...
  bool Set_Maxspeed_Command(
    ...
    auto parameters = std::vector<rclcpp::Parameter>();
    parameters.emplace_back(
        rclcpp::Parameter("FollowPath.desired_linear_vel", request->max_speed));
    set_speed_parameter_client_->set_parameters(parameters);
    ...
    if (_pDynamic_param.m_bFlag_onetime) {
      if (feedback->distance_remaining < 0.1 &&
        ...
        tf2::Quaternion goal_q(
            _pGoal_pose.goal_quarterX, _pGoal_pose.goal_quarterY,
            _pGoal_pose.goal_quarterZ, _pGoal_pose.goal_quarterW);
        tf2::Matrix3x3 goal_m(goal_q);
        double goal_roll, goal_pitch, goal_yaw;
        goal_m.getRPY(goal_roll, goal_pitch, goal_yaw);

        tf2::Quaternion robot_q(
            footprint.orientation.x, footprint.orientation.y,
            footprint.orientation.z, footprint.orientation.w);
        ...
        {
          parameters.emplace_back(rclcpp::Parameter(
              "FollowPath.rotate_to_heading_angular_vel", 0.1));
          parameters.emplace_back(
              rclcpp::Parameter("FollowPath.rotate_to_heading_min_angle", 0.1));
        } else if (angle_diff <= 0.785398) // 45도 이하일 때만 적용
        {
          parameters.emplace_back(rclcpp::Parameter(
              "FollowPath.rotate_to_heading_angular_vel", angle_diff));
          parameters.emplace_back(rclcpp::Parameter(
              "FollowPath.rotate_to_heading_min_angle", angle_diff));
        } else {
          parameters.emplace_back(rclcpp::Parameter(
              "FollowPath.rotate_to_heading_angular_vel", 0.785398));
          parameters.emplace_back(rclcpp::Parameter(
  ...
  void LedControl_Call(int id, int led_brightness) {
    ...
    auto result_future = led_control_client->async_send_request(
        request, std::bind(&TETRA_SERVICE::LedControl_callback, this,
                           std::placeholders::_1));
  }

  void LedToggleControl_Call(int de_index, int light_accel,
                             int led_high_brightness, int light_decel,
                             int led_low_brightness) {
    auto request =
        std::make_shared<interfaces::srv::LedToggleControl::Request>();
    ...
    auto result_future = led_toggle_control_client->async_send_request(
        request, std::bind(&TETRA_SERVICE::LedToggleControl_callback, this,
                           std::placeholders::_1));
  }
  ...
  void ToggleOn_Call(int id) {
    ...
    auto result_future = toggle_on_client->async_send_request(
        request, std::bind(&TETRA_SERVICE::ToggleOn_callback, this,
                           std::placeholders::_1));
  }
  ...
  void ImuReset_Call() {
    ...
    auto result_future = imu_reset_client->async_send_request(
        request, std::bind(&TETRA_SERVICE::ImuReset_callback, this,
                           std::placeholders::_1));
  }

  bool Clear_Costmap() {
    // Check if the service is available
    if (!clear_entire_global_costmap_client->wait_for_service(
            std::chrono::seconds(1))) {
      RCLCPP_ERROR(this->get_logger(),
                   "Clear global_Costmap Service not available.");
      return false;
    }

    if (!clear_entire_local_costmap_client->wait_for_service(
            std::chrono::seconds(1))) {
      RCLCPP_ERROR(this->get_logger(),
                   "Clear local_Costmap Service not available.");
      return false;
    }
    ...
    auto request2 =
        std::make_shared<nav2_msgs::srv::ClearEntireCostmap::Request>();
    auto future =
        clear_entire_global_costmap_client->async_send_request(request);
    auto future2 =
        clear_entire_local_costmap_client->async_send_request(request2);
...
private:
  ...
  rclcpp::Client<interfaces::srv::LedToggleControl>::SharedPtr
      led_toggle_control_client;
  ...
  rclcpp::Service<interfaces::srv::RunNavigation>::SharedPtr
      run_navigation_cmd_srv;
  rclcpp::Service<interfaces::srv::GetInformation>::SharedPtr
      get_information_cmd_srv;
  rclcpp::Service<interfaces::srv::SetLocation>::SharedPtr set_location_cmd_srv;
  rclcpp::Service<interfaces::srv::GotoLocation>::SharedPtr
      goto_location_cmd_srv;
  rclcpp::Service<interfaces::srv::GotoLocation2>::SharedPtr
      goto_location2_cmd_srv;
  ...
  while (rclcpp::ok() && !stop_requested) {
    ...
    if (m_bActive_map) {
      if (m_iPose_reset_cnt > 500) {
        ...
        if (_pRobot.m_iCallback_Charging_status == 2 ||
            _pRobot.m_iCallback_Charging_status == 3 ||
            _pRobot.m_iCallback_Charging_status == 6 ||
            _pRobot.m_iCallback_Charging_status == 7) {
          m_iPose_reset_cnt++;
