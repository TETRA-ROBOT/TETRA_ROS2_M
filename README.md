...
### 1. ROS2 Jazzy 설치
Ubuntu 24.04(noble) 기준, [공식 ROS2 설치 가이드](https://docs.ros.org/en/jazzy/Installation/Ubuntu-Install-Debs.html)를 참고하세요.

### 2. 현재 사용자를 연결된 센서 장치 그룹에 추가
아래 명령으로 현재 사용자를 dialout 그룹에 추가하세요:
```bash
ls -l /dev/ttyUSB0 # 장치의 그룹 확인 (crw-rw---- 1 root dialout 188, 1 Apr 28 14:07 /dev/ttyUSB1)
sudo usermod -a -G dialout $USER # 그룹 추가
groups # 그룹 확인 (재로그인해야함, 안할려면 newgrp dialout 명령)
```
